...
The cost is a few megabytes read every five minutes, off the main thread, with no network, no privilege and no subprocess. Pressing **Restart** drops what it had learned, so the fix clears the warning on the next tick rather than several minutes later.
