...
| `9ecf0a8` | The stealth sub-page — 26 settings, keys proven against the real file |
...
**§2 is empty — every item planned for tonight has landed.**

What remains needs a decision from §5, which is not an agent's to make.
...
- **Superseded, and this is now the strongest tool available.** Naming the sidebar rows made them selectable over AT-SPI, so any page can be opened headlessly and read. The recipe: find the node with role `list` (not `list box`), `get_selection_iface().select_child(n)`, then walk names. This proved the Stealth page's five groups and the dependency caveat in both directions — present when unmet, gone when met.
