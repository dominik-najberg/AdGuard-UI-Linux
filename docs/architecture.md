...
- **The one filter action that destroys something is not built.** `filters remove` on a custom filter deletes the row outright, where the same command against a catalogue filter merely clears `is_installed`. That asymmetry is why turning a switch off is `disable` everywhere in this app, and why removal wants a confirmation of its own rather than a quiet suffix button.
...
Status: Status, Protection, Filters (HTTP), DNS, Advanced and Stealth are done; both filter pages install custom lists by URL (§5); licence activation lives on the Status page; the first-run assistant seeds an unconfigured install and hands the window to the pages when it is finished (§5); the tray carries start/stop plus the six Protection toggles as quick toggles (§4); and `--background` plus the autostart entry put that tray on screen at login without a window. Still open for v1: auto-mode detection, the reconcile toast (§3), and removing a custom filter — the one destructive filter action, deliberately left out of the install change.
