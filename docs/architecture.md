...
| Privileged ops | **None of our own** | AdGuard already ships the root helper `auto` mode needs; we detect and instruct. See §6. The one thing the app does perform — clearing a proxy process the CLI has lost track of — needs no privilege, because the process is the user's own. |
...
| --- | --- | --- |
| **Status** | Hero panel: protection on/off, one primary action, restart; three at-a-glance figures; HTTP + SOCKS5 endpoints; what is being filtered; licence state. Every figure and row but the licence links to the page that owns the setting behind it | `status`, `license`, plus `proxy.yaml` and `agflm_*.db` for the figures |
