...
> **This stops the proxy you are running.** Measured 2 August 2026: starting the app under a throwaway `$XDG_DATA_HOME` is followed within about a second by a clean `AGProxyServer::stop()` in the **real** daemon's `~/.local/share/adguard-cli/logs/proxy.log`, four times out of four — three on 2 August 2026 and a fourth on 6 August, which was expected before it happened and cost nothing because it was. `$XDG_DATA_HOME` redirects the *configuration*; the daemon is a singleton on the machine and there is one of it regardless. Sandboxed `config get`/`config set` are **not** affected — only launching the app. Check `adguard-cli status` when you are done and `adguard-cli start` if it is down, because the user unit is `active (exited)` with `MainPID 0` and will not. `handoff.md` §3 item 11 has the full measurement and the bisection that would identify which part of the app is responsible.
...
**Keep the sandbox path short, and `/tmp/fake` is short on purpose.** If you ever `adguard-cli start` inside one — the trust measurement (contract §6) needed a proxy of its own, not just a config — the data directory has to leave room for the control socket underneath it. A Unix socket path cannot exceed **107 bytes**, and a sandbox under a session scratch directory blew past it: `create_sockaddr_un: Socket name length 144 exceeds maximum allowed length 107`, then `Failed to init control socket`, surfacing at the prompt as the wholly uninformative `Failed to start proxy server: An unknown error has occurred`. **Nothing in that message says path length**, so budget for it rather than diagnose it: a couple of levels under `/tmp` and no deeper. The failure only appears when a proxy starts, which is why every recipe above it never met the limit.
...
**Export `GDK_BACKEND=x11` for the app as well, or unsetting the two display variables is not enough.** Measured 15 August 2026 while re-taking the 1.5.0 set. With `env -u DISPLAY -u WAYLAND_DISPLAY` on the way in and `DISPLAY=:99` exported inside, the app still did not put a window on `:99` — `xwininfo -root -tree` listed the root window with **no children at all**, while the same run's AT-SPI walk found the frame, walked the sidebar and named all eight pages. Adding `GDK_BACKEND=x11` to the app's own environment and changing nothing else put the window on `:99` at `880x720+0+0`; that A/B was run twice, one toplevel against zero. Why GDK declines the X server it was handed was not pinned down here, so take this as the measurement it is rather than a mechanism — `wayland-0` is still sitting in `$XDG_RUNTIME_DIR` whatever `$WAYLAND_DISPLAY` says, and the obvious experiment (point `$XDG_RUNTIME_DIR` at an empty directory) breaks the session bus and proves nothing.

This is the black-frame trap below wearing a better disguise, and it is worth knowing the two apart: that one grabs an empty screen and returns a frame with an X cursor in it, and this one returns a frame that is *entirely* empty while every accessibility probe passes and reports the page it was asked for. A harness that checks its walk and not its pixels calls this a clean run.
...
**All three were spliced for 1.5.0, the Stealth address included, and splicing is now the method rather than the shortcut.** Repainting is what you fall back to when there is no reviewed frame to copy from — a first capture, or a row whose layout genuinely moved. Everywhere else the paste is strictly better, and it gets better the longer the placeholders stay put: `user@example.com`, a key ending `A1B2` and `203.0.113.42` have been the committed values since 1.0.0, and each re-take that copies them forward is one more release in which no real value was ever in a file that could be committed by accident. Splice generously — a band a few rows taller than the text costs nothing, because every other pixel in it is identical anyway — and let the assert be the thing you trust. The check that the splice landed and the check that nothing *else* moved are the same comparison: after the paste, the content column should differ from the committed frame only where this machine's state legitimately differs, and on the 1.5.0 re-take that was two places in the whole set — a web-filter count of 7 where the old frame said 8, and the About page's version and timestamp.
...
**An empty `$ADGUARD_BROWSER_HOME` deletes the group you came to photograph.** Measured 2 August 2026 while re-taking the unmet frame for 1.1.0: pointing it at a bare empty directory produces a frame with no browser-integration group at all, because no browser is *installed* under it and this application does not report browsers that are not there (`architecture.md` §6). The frame that group appears in needs `.config/google-chrome` and `.config/chromium` to **exist and be empty** — installed, no manifest. `/tmp/adguard-demo/` is the root the committed frame uses, and it is worth reusing: the path is rendered into the picture, so a scratch directory with a session id in its name ends up on a public README.

**Four more things measured while re-taking the 1.1.0 set**, each of which reads as a bug in the app rather than in the harness:

- **A page longer than the window is the normal case now, not the exception.** The DNS and Filters pages carry their whole catalogues — about 5,000 and well over 5,200 pixels on this machine — so "resize the window to the page and crop" produces a strip nobody will look at. The committed frames are the *top* of each page at a 1,400 px window, which is what the 1.0.0 set already did; `README.md` says so beneath them rather than leaving a reader to think a page is all of it.
- **The bottom rows of the frame are window chrome, not page content.** With no window manager, GTK leaves two 1 px lines at the very bottom edge. A crop that scans up from the last row for "the last row that is not background" finds one of them and reports *every* page as exactly full height — which looks like a page that overflows and is nothing of the kind. Start the scan about ten rows up.
- **`Atspi.Component.scroll_to` fails here** with the same bare `atspi_error` that `handoff.md` §4 records for `grab_focus`. That is the obvious way to frame a group below the fold and it does not work. It does not need to: with no window manager the window can be made as tall as the whole page, which turns a scrolled viewport into a *crop* — take one tall frame, read the group's `y` off `Atspi.Component.get_extents`, and cut. `docs/screenshots/advanced-backup.png` was made that way.
- **Cut the crop on a background row, not at a fixed offset.** Both edges: a fixed offset above a group heading slices the previous group's last row in half, and a frame that ends mid-row reads as a rendering fault rather than as a page continuing. Walking to the nearest all-background row costs four lines and is the difference between a frame that looks deliberate and one that looks broken.
...
**A walk cannot prove what a second, faster click would do**, and that is a limit of the channel rather than of the walker. `Atspi.Action.do_action` is a D-Bus round trip, so a modal dialog raised by the first press is up before the second is delivered: six presses on the Filters page's removal button produce one dialog whether or not the button desensitises its row ([issue #5](https://github.com/dominik-najberg/AdGuard-UI-Linux/issues/5), `architecture.md` §5, measured 6 August 2026). Anything whose defect lives in the gap between an event and the future that handles it is outside this harness by construction, and pressing harder does not narrow the gap. Read the handler and argue from the widget's event semantics instead — a clean walk is evidence about the slow path and about nothing else.
...
```bash
cargo test --workspace --locked                 # 396 pass, 67 ignored
...
3. **Add the release to `data/…metainfo.xml`**, which is what GNOME Software renders, and **bump the `.deb` filename in `README.md`'s install example**. Those are the two other files carrying a version number — this step used to say metainfo was the only one, which was wrong from the day the README was written and stayed wrong through 1.0.0. The metainfo entry carries a description as well; the README's is a filename a reader will copy, so a stale one sends them to `apt` with a package that is not in the release.
   ...
   **Screenshots are part of this step when the pages moved.** `docs/screenshots/` is ten frames and `README.md` embeds all ten; a release that adds settings to a page ships a README picturing the page without them. The recipe is §3 above, and the frames to re-take are the pages that changed — the Status and Stealth frames carry placeholders over this machine's own licence and IP, so re-taking one that did not need it is how a real address reaches a public repository.

   **"The pages that changed" is not the same set as "the frames that changed", and reading it as one cost this project eight stale frames.** Measured 15 August 2026, re-taking the 1.5.0 set: **the sidebar is in every full-window frame**, so a release that adds a *page* invalidates all of them at once, however untouched the pages themselves are. 1.3.0 added About and 1.5.0 added Extensions; each time the frame for the new page was taken and the rest were left, because no other page had changed — and each time every other frame kept a sidebar that was missing a row. The rule that catches it: a new sidebar entry re-takes the whole set, a changed page re-takes that page. The one exception is `advanced-backup.png`, which is cropped to the content column and has no sidebar in it — it survives a page being added, and it was left alone for exactly that reason.

   **Re-taking a frame does not have to mean re-deciding its framing.** Crop each new frame to the *committed* frame's height instead of recomputing the cut. On an unchanged page the content column comes back pixel-identical between runs — that was checked, and Protection, DNS, Filters and the unmet frame all compared equal outside the sidebar — so the reviewed framing is still the right one, and reusing it makes the diff provably a sidebar change and nothing else. It also sidesteps the crop rule's tuning entirely, which is worth something: the rule below has to be told where the window border and the scrollbar sliver are before "the whole row is background" is ever true.

   **One frame cannot be re-taken here, and the README says why rather than going stale quietly.** The Filters page gained the trusted-list padlock, and that control only renders on a *custom* row; this machine subscribes to no custom lists, so `filters.png` shows the *Custom filters* group holding nothing but its URL entry — accurate, and with no padlock in it. Installing someone's list to photograph it would put a third party's rules on the machine the release is cut from. **Leave it** unless the release is cut somewhere that has one, and leave the README's note beside it explaining the absence; a frame that is right for a reason beats a frame that is right by accident.
...
| [`architecture.md`](architecture.md) | Design of the GUI: crates, data flow, UI structure, privilege model. **§7 is the scope authority for both milestones** — v1 closed, v2 open |
...
| [`v2-plan.md`](v2-plan.md) | **Live.** The reasoning behind v2's scope and the standing rules for working under it — *not* the scope itself, which moved to `architecture.md` §7 when this file's own §2 was carried out on 2 August 2026. Still queues no code. Pins nothing that moves — ground truth stays in `handoff.md` §0 |
| [`overnight-v2.md`](overnight-v2.md) | **Live.** How a session behaves on v2 when nobody is awake to answer it — the order to take the three items in, what an unattended run may not do, and when to stop. Adds no scope; `architecture.md` §7 is still the authority |
