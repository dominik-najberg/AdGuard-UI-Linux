...
cargo test --workspace      # 226 pass, 44 ignored
...
- **Pick a v2 item**, from `architecture.md` §7's *v2 — open* half. Cheapest first, which is also the order §7 reasons in — and the first of the three has now had its first task taken:
  ...
  **Seven of the eleven proposed rows are built and none of them should be built again**: the five `https_filtering.*` booleans, which are the Advanced page's *HTTPS filtering* group; `dns_filtering.block_ech`, which is DNS ▸ *Browser compatibility*; and `safebrowsing.send_anonymous_statistics`, which is Protection ▸ *Privacy*. So the live figures are **65 rendered and 15 not**, and **four rows are outstanding, of which only one is straightforwardly buildable**: `adguard_headers_enabled` in Advanced ▸ Diagnostics is the next one to take. `auto_enable_language_filters` turned out to be **blocked on a measurement that needs a second proxy** (§3 item 12) — investigating it refuted both halves of what the enumeration had said about it, so read that before touching it — and `filtered_ports` and `outbound_interface` each need a design decision first. **What is left of this item is rows**, not specification, plus the three smaller decisions in §3 item 8.
...
**226 tests pass by default** and 44 more are `#[ignore]`d.
