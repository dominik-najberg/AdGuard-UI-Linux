...
- **Pick a v2 item**, from `architecture.md` §7's *v2 — open* half. Cheapest first, which is also the order §7 reasons in — and the first of the three has now had its first task taken:
  ...
  **Seven of the eleven proposed rows are built and none of them should be built again**: the five `https_filtering.*` booleans, which are the Advanced page's *HTTPS filtering* group; `dns_filtering.block_ech`, which is DNS ▸ *Browser compatibility*; and `safebrowsing.send_anonymous_statistics`, which is Protection ▸ *Privacy*. So the live figures are **65 rendered and 15 not**, and **three rows are outstanding, none of which is straightforwardly buildable**. `auto_enable_language_filters` is **blocked on a measurement that needs a second proxy** (§3 item 12) — investigating it refuted both halves of what the enumeration said about it, so read that before touching it. `filtered_ports` and `outbound_interface` each need a **design decision first**: the compound range syntax `80:5221,5300:49151` is ours to validate because `config set` type-checks strings not at all, and how an empty text field writes back `null` is unmeasured. **So the cheap half of this item is done and what is left is one blocked measurement and two design calls** — which makes it, for the first time, no longer the obvious thing to pick up. **What is left of this item is rows**, not specification, plus the three smaller decisions in §3 item 8.
