...
- **Pick a v2 item**, from `architecture.md` §7's *v2 — open* half. Cheapest first, which is also the order §7 reasons in — and the first of the three has now had its first task taken:
  ...
  **So this item is down to one blocked row plus the three smaller decisions in §3 item 8** — and the three things worth carrying out of it are general rather than per-row: the enumeration's *Where it belongs* column has now been overruled by `proxy.yaml` three times, a CLI refusal is not evidence about what would be accepted (§4), and an assertion that fails when a new row is added is the guard working rather than an obstacle.
...
8. **What the parity enumeration left open** — four questions, opened 2 August 2026 by `architecture.md` §5's *What the pages do not render*. None blocks the eleven rows that enumeration proposes; each is small, and each is here so it is not rediscovered.
   ...
   - **`show_notifications` needs one measurement, and the answer decides the shape.** `proxy.yaml` says only *"show protection status notification"*. If those are desktop notifications it collides with this app's own tray and is a design question; if it is terminal output it belongs beside `show_hints` and stays out. **`adguard-cli.md` already glosses it as "desktop notifications" — that gloss is ours and is unmeasured**, and it should not be cited as though it settled the question.
   - **ECH is handled on two pages.** `https_filtering.encrypted_client_hello` is on Advanced; its counterpart `dns_filtering.block_ech` would land on the DNS page. Cheap to resolve now, annoying later.
