...
cargo test --workspace      # 222 pass, 44 ignored
...
- **Pick a v2 item**, from `architecture.md` §7's *v2 — open* half. Cheapest first, which is also the order §7 reasons in — and the first of the three has now had its first task taken:
  ...
  **Six of the eleven proposed rows are built and none of them should be built again**: the five `https_filtering.*` booleans, which are the Advanced page's *HTTPS filtering* group, and `dns_filtering.block_ech`, which is DNS ▸ *Browser compatibility*. So the live figures are **64 rendered and 16 not**, and **five rows are outstanding**: `safebrowsing.send_anonymous_statistics` on Protection, `auto_enable_language_filters` on Filters, `adguard_headers_enabled` in Advanced ▸ Diagnostics, and — each needing a design decision first — `filtered_ports` and `outbound_interface`. **What is left of this item is rows**, not specification, plus the three smaller decisions in §3 item 8.
...
**222 tests pass by default** and 44 more are `#[ignore]`d.
