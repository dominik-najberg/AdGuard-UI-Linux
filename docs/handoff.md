...
cargo test --workspace      # 236 pass, 44 ignored
...
- **Pick a v2 item**, from `architecture.md` §7's *v2 — open* half. Cheapest first, which is also the order §7 reasons in — and the first of the three has now had its first task taken:
  ...
  **Nine of the eleven proposed rows are built and none of them should be built again**: the five `https_filtering.*` booleans, which are the Advanced page's *HTTPS filtering* group; `dns_filtering.block_ech`, which is DNS ▸ *Browser compatibility*; `safebrowsing.send_anonymous_statistics`, which is Protection ▸ *Privacy*; `adguard_headers_enabled`, which is a row in Advanced ▸ *Diagnostics*; and `filtered_ports`, which is Advanced ▸ *Filtered ports*. So the live figures are **67 rendered and 13 not**, and **the two rows still outstanding have both been measured, which is what this item's remaining work turned out to be.** Neither is blocked on wanting to; each is blocked on something specific:

  - **`auto_enable_language_filters` — blocked on a measurement that needs a second proxy** (§3 item 12). Investigating it refuted both halves of what the enumeration said about it, so read that before touching it.
  - **`filtered_ports` — built 2 August 2026**, as Advanced ▸ *Filtered ports*, its own group directly above *Manual proxy ports* and worded to mirror it. This file used to say its range syntax was ours to validate because `config set` type-checks strings not at all. **Refuted by thirty-nine sandboxed writes**: the CLI enforces a real grammar — descending ranges, `65536`, `80:`, `-1` and the empty string all refused. What was ours turned out to be the *wording*, because **the CLI's refusal recommends the one form it rejects** (it says *space-separated*; `80 443` is refused). So this is the one row on that page that does not let the CLI word its own refusal, and three tests keep it that way. Contract §5, `architecture.md` §5.
  - **`outbound_interface` — the real blocker, and it is not the one this file named.** The write half was the open question and it is trivial (write the word `null`, which restores the stock line byte-identically; `""` leaves the CLI and every YAML reader disagreeing). The **read** half stops it: `str_at` returns `None` for a null, the page renders `None` as *unavailable*, and this is the only null-valued scalar in the file. Adding it as a `Kind::Text` was tried, and `every_advanced_setting_resolves_with_the_right_type` **fails on a stock install**. So the row costs `Kind::Text` a notion of *absent* distinct from *unreadable* — a shared type behind nine existing rows. `architecture.md` §5.

  **So this item is no longer the obvious thing to pick up, and that is now a measured statement rather than an impression.** One of the three is genuinely buildable next (`filtered_ports`), one needs a type change worth doing deliberately, one needs a machine-wide change. Plus the three smaller decisions in §3 item 8.
...
13. **`Kind::Text` cannot express "legitimately empty", and `outbound_interface` is the row that needs it.** Opened 2 August 2026 by measurement, and it is the last parity row that needs a decision rather than a measurement — but the decision is **ours, not the owner's**, which is why it is here rather than quoted upward.

    **What was measured.** `Config::str_at` returns `Some` only for `Yaml::String`, so a null key reads as `None`; the Advanced page renders `None` as *unavailable*, which is the state reserved for a key it could not read at all — a wrong type from a hand edit. `outbound_interface` is the **only null-valued scalar in the whole 220-line file**, so nothing in the reader has ever had to tell the two apart. Adding the row as a plain `Kind::Text` was tried rather than reasoned about, and `every_advanced_setting_resolves_with_the_right_type` fails against the real config on this machine — `PROBE (outbound_interface) did not resolve as Text { secret: false }`. **That assertion is the guard working**, not an obstacle to route around: it exists so a table-driven row cannot ship rendering as broken on a stock install.

    **The write half needs nothing.** Contract §5 has it: writing the literal word `null` restores the stock line byte-identically, while writing `""` leaves an empty scalar that every YAML reader calls null and the CLI itself reads back as an empty string. A clear button writes `null`.

    **What the decision costs.** `Kind::Text` is behind nine rows, three of which are credentials that legitimately hold `""` — so the new state is *absent*, and it is not the same as empty. Whatever shape it takes has to keep `str_at`'s current callers reading exactly as they do now, and the live-config assertion has to keep failing for a genuinely unreadable key while passing for a null one. **Do not widen `str_at` itself to swallow nulls**: that would make every existing row silently tolerant of a key it should still call unavailable, which is the whole point of the assertion above.
