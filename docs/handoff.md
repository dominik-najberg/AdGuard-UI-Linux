...
cargo test --workspace      # 219 pass, 44 ignored
...
- **Pick a v2 item**, from `architecture.md` §7's *v2 — open* half. Cheapest first, which is also the order §7 reasons in — and the first of the three has now had its first task taken:
  ...
  **The five `https_filtering.*` booleans were the obvious first slice and are now the Advanced page's *HTTPS filtering* group** — do not build them again. That leaves **six** of the eleven proposed rows outstanding, so the live figures are 63 rendered and 17 not. Next by the same reasoning: `dns_filtering.block_ech` on the DNS page, `safebrowsing.send_anonymous_statistics` on Protection, `auto_enable_language_filters` on Filters, `adguard_headers_enabled` in Advanced ▸ Diagnostics, and — each needing a design decision first — `filtered_ports` and `outbound_interface`. **What is left of this item is rows**, not specification, plus the three smaller decisions in §3 item 8.
...
**219 tests pass by default** and 44 more are `#[ignore]`d.
