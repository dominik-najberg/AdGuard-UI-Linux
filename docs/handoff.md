...
- **Pick a v2 item**, from `architecture.md` §7's *v2 — open* half. Cheapest first, which is also the order §7 reasons in — and the first of the three has now had its first task taken:
  ...
  **Seven of the eleven proposed rows are built and none of them should be built again**: the five `https_filtering.*` booleans, which are the Advanced page's *HTTPS filtering* group; `dns_filtering.block_ech`, which is DNS ▸ *Browser compatibility*; and `safebrowsing.send_anonymous_statistics`, which is Protection ▸ *Privacy*. So the live figures are **65 rendered and 15 not**, and **four rows are outstanding**: `auto_enable_language_filters` on Filters, `adguard_headers_enabled` in Advanced ▸ Diagnostics, and — each needing a design decision first — `filtered_ports` and `outbound_interface`. **What is left of this item is rows**, not specification, plus the three smaller decisions in §3 item 8.
