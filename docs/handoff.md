...
- **Pick a v2 item**, from `architecture.md` §7's *v2 — open* half. Cheapest first, which is also the order §7 reasons in — and the first of the three has now had its first task taken:
  ...
  **Import/export** is the largest, and **it is now measured but not yet designed** — contract §13, new on 2 August 2026, is the whole of what the three commands actually do. The design §7 requires still has to be written; §3 item 10 lists the four facts it has to answer to. The headline: `import-settings` really does create `proxy.yaml` on a virgin install (so the first-run collision is real, confirmed rather than inferred), the install it leaves is **unlicensed and has no certificate** while `proxy.yaml` says HTTPS filtering is on, and **the two exports share one filename** so feeding `import-settings` a logs zip succeeds at exit 0 with identical wording and a partial install. `v2-plan.md` §3 is the working behind all three items.
...
10. **Import/export is measured and undesigned, and the design is the next thing it needs.** Contract §13 has the measurements, taken 2 August 2026; `architecture.md` §7 requires the first-run collision to be designed before either half is built, and `overnight-v2.md` §2.3 keeps the build itself off an unattended run. Four measured facts the design has to answer to, none of which is a coding question:

    - **An import onto an unconfigured install leaves it unlicensed and without a certificate**, while the `proxy.yaml` it just wrote says `https_filtering.enabled: true`. So the import path lands the user in exactly the state §5 says must be marked — and unlike the assistant's route, nothing has offered to activate or to trust anything. What the first-run screen does when handed an import is the decision.
    ...
    - **`export-settings` carries no licence and no CA private key** — `adguard.conf` is not in it. That makes an export safe to share, which is the fact the export button's wording should rest on. It also means an import can never restore a licence.
    - **`export-logs` discloses `proxy.yaml`** and does **not** contain `access.log`. The button's text has to say the first; this project's own prior sentence said the opposite of both and is struck in `overnight-v2.md` §2.3.

    Two smaller ones for whoever builds it: `export-settings` is 99 % `agflm_standard.db` (51 MB of 51.8 MB), so it is slow for a reason that is not the user's settings; and the DNS catalogue and `dns_user.txt` are **not exported at all**, so a round trip silently loses DNS filter selections and user rules.
