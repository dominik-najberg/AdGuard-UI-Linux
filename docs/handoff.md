...
1. **DNS page.** `FilterSet::Dns` already carries the CLI prefix, the database path and the user-rules path, and `FiltersPage` is parameterised by it, so the catalogue half is close to free. The rest is not: the upstream/fallback/bootstrap lists need `config list-add`/`list-remove`, which `cli.rs` does not have (`Config::list_at` is only the read half), and the user-rules toggle cannot go through `dns filters enable` (contract §6). The page also owns `dns_filtering.listen_port` — disabled (`-1`), automatic (`0`) or fixed (`N`, 1..65535), which is what finally cures the inert-DNS caveat Protection has been showing with no remedy anywhere in the app. **Measure which address that listener binds before shipping the row**; unmeasured, and `listen_address`'s precedent says a non-loopback bind needs confirmation and a standing warning.
