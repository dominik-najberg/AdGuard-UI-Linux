...
- **Pick a v2 item**, from `architecture.md` §7's *v2 — open* half. Cheapest first, which is also the order §7 reasons in — and the first of the three has now had its first task taken:
  ...
  **HAR capture** is an Advanced-page group over two keys with two open points: the subtitle naming what leaving capture on costs, and the stock `location` of `'.'`, which §7 makes that item's *first* task. That task **has moved but is not finished** — contract §9 now records the proxy's working directory (`/home/potworny`, and inherited from whoever launched it) and shows that AdGuard resolves the file's other relative paths against **per-key base directories rather than one rule**, so `.` still cannot be predicted and the shortcut of generalising from `access_log_file` is now known to be wrong. Producing an actual HAR dump is still the measurement.
...
9. **Where `har_writer.location`'s `'.'` resolves is still unmeasured**, and §7 makes it that item's first task. What was settled on 2 August 2026 is only that it **cannot be inferred** — contract §9 has the working: the proxy's cwd is `/home/potworny` and is inherited from its launcher rather than chosen, while `access_log_file: 'access.log'` lands in `<data>/logs/` and is therefore not cwd-relative at all. Three relative paths, no single base. **Producing an actual HAR dump is the remaining measurement**, and it is a write plus a proxy restart, so it wants a sandbox and a plan rather than a spare five minutes.
