...
- **Pick a v2 item**, from `architecture.md` §7's *v2 — open* half. Cheapest first, which is also the order §7 reasons in — and the first of the three has now had its first task taken:
  ...
  **Advanced parity's enumeration is done** (2 August 2026, `architecture.md` §5, *What the pages do not render*). §7 required it before any row, and it is not code. **80 leaf keys in `proxy.yaml`, 58 rendered, 22 not** — and of the 22, eleven should become rows, seven should stay unrendered with a reason each, two are `har_writer`'s and belong to the other item, and two cannot be classified without one more measurement apiece. The obvious first slice is the five `https_filtering.*` booleans, which are one coherent group sharing one dependency the Advanced page already knows how to state. **What is left of this item is now rows**, not specification, plus the three smaller decisions in §3 item 8.
