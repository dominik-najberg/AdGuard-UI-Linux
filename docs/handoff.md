...
cargo test --workspace      # 228 pass, 44 ignored
...
- **Pick a v2 item**, from `architecture.md` §7's *v2 — open* half. Cheapest first, which is also the order §7 reasons in — and the first of the three has now had its first task taken:
  ...
  **Eight of the eleven proposed rows are built and none of them should be built again**: the five `https_filtering.*` booleans, which are the Advanced page's *HTTPS filtering* group; `dns_filtering.block_ech`, which is DNS ▸ *Browser compatibility*; `safebrowsing.send_anonymous_statistics`, which is Protection ▸ *Privacy*; and `adguard_headers_enabled`, which is a row in Advanced ▸ *Diagnostics*. So the live figures are **66 rendered and 14 not**, and **the three rows still outstanding have all now been measured, which is what this item's remaining work turned out to be.** None of the three is blocked on wanting to; each is blocked on something specific:
  ...
  - **`filtered_ports` — buildable, and the decision it was waiting for does not exist.** This file used to say its range syntax was ours to validate because `config set` type-checks strings not at all. **Refuted 2 August 2026 by twenty-nine sandboxed writes**: the CLI enforces a real grammar — descending ranges, `65536`, `80:`, `-1` and the empty string all refused. What is left is not validation but wording, because **the CLI's refusal recommends the one form it rejects** (it says *space-separated*; `80 443` is refused). The row states the grammar in `proxy.yaml`'s words, never the binary's. Contract §5.
