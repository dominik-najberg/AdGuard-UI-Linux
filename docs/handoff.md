...
- **Pick a v2 item**, from `architecture.md` §7's *v2 — open* half. Cheapest first, which is also the order §7 reasons in — and the first of the three has now had its first task taken:
  ...
  **Ten of the eleven proposed rows are built and none of them should be built again**: the five `https_filtering.*` booleans, which are the Advanced page's *HTTPS filtering* group; `dns_filtering.block_ech`, which is DNS ▸ *Browser compatibility*; `safebrowsing.send_anonymous_statistics`, which is Protection ▸ *Privacy*; `adguard_headers_enabled`, which is a row in Advanced ▸ *Diagnostics*; `filtered_ports`, which is Advanced ▸ *Filtered ports*; and `outbound_interface`, which is Advanced ▸ *Outgoing connections*. So the live figures are **68 rendered and 12 not**, and **one row is left, and it is blocked on a machine-wide change rather than on effort.**

  **`auto_enable_language_filters` is the one row left, and it is blocked on a measurement that needs a second proxy** (§3 item 12) — the same wall as HAR capture, so whatever authorises one almost certainly authorises the other and they should be quoted to the owner together. Investigating it refuted **both halves** of what the enumeration said about it, so read that before touching it.
  ...
  **HAR capture is blocked on the owner, and that is now a finding rather than a pause.** §7 makes *where `location: '.'` resolves* the item's first task, and §2.2 of `overnight-v2.md` fixes the order so the subtitle waits behind it. Contract §9 now shows the answer cannot be inferred — the proxy's cwd is `/home/potworny`, inherited from whoever launched it, while AdGuard resolves the file's other relative paths against **per-key base directories rather than one rule**. Measuring it needs a second proxy, which spawns a second root helper (contract §8) on a machine whose real proxy is in `auto` mode — a machine-wide change, so §3 item 9 carries it with the recipe an authorised run would follow. **The write half is measured and needs nobody**: `har_writer.location` is unvalidated in every respect and stores `~` literally, so the row must expand or refuse it.
...
9. **Where `har_writer.location`'s `'.'` resolves is still unmeasured, and it is blocked on the owner rather than on effort.** §7 makes it the HAR item's first task. Two things were settled on 2 August 2026 and they point in opposite directions.
...
12. **`auto_enable_language_filters` needs one measurement before it can be a row, and that measurement needs a second proxy.** Opened 2 August 2026 after four parallel investigations, three of which contradicted what `architecture.md` §5 had recorded. The corrected reasoning is there; this is the blocker.
