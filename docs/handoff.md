...
4. ~~**The sidebar rows have no accessible name.**~~ Fixed — each row now carries an accessible label. Recorded because of what it unblocked: with the rows named, a headless test can select a page over AT-SPI and read its contents, so "the page renders" stopped being a claim. Original finding: Found while trying to drive the UI headlessly: the AT-SPI tree exposes 133 nodes and names every `AdwActionRow` on the visible page, but the five navigation rows come through unnamed, because each is a bare `GtkListBoxRow` holding an icon and a label. A screen-reader user cannot tell the entries apart, and it is also what stops any headless test from opening a page other than Status — so fixing it buys accessibility *and* unblocks verification for every page after this one. `set_accessible_label` on the row, or making the label the row's labelled-by.
