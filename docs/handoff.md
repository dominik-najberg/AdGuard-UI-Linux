...
- **Watch what a public repository brings**, which is the one genuinely new category of work since 1.0.0: issues from people whose machines are not this one. Every measurement in `cli-contract.md` was taken against `adguard-cli` 1.4.13 on Ubuntu 26.04 with GNOME 50 on Wayland, and the first bug report from anywhere else is worth more than any of the v2 backlog — it is the only way to find out which of this project's constants were facts about the CLI and which were facts about this machine.
